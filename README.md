...
computes the time evolution of electromagnetic cavity modes driven by
the effective Gertsenshtein current generated by primordial black hole
(PBH) mergers or early inspiral signals. The repo also contains tools to compute **GW–cavity coupling coefficients**.
...
-   TE and TM electromagnetic modes\
-   Arbitrary gravitational-wave direction\
-   Chirp (merger) and monochromatic (inspiral) signals
...
c''\_n(t) + (ω_n / Q) c'\_n(t) + ω_n² c_n(t) = - I_n(t)

where the driving term is

I_n(t) = ∫ dx_parallel A_plus(x_parallel) ∂²_t h_plus(t -
x_parallel/c) + ∫ dx_parallel A_cross(x_parallel) ∂²_t h_cross(t -
x_parallel/c)
